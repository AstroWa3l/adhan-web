...
def monitor_and_update_prayer_times(latitude, longitude, timezone, method, sound_file):
    ...
    """
    print("Monitoring prayer times... Press Ctrl+C to stop.")
    current_date = date.today()  # Track the current date

    # Calculate initial prayer times
    prayer_times = calculate_prayer_times(latitude, longitude, timezone, method)
    print(f"Today's Prayer Times for Acton, CA ({current_date}):")
    for prayer, time_value in prayer_times.items():
        if prayer != "date":
            print(f"{prayer.capitalize()}: {time_value}")
    ...
    try:
        while True:
            ...
            if now.date() > current_date:
                ...
                prayer_times = calculate_prayer_times(latitude, longitude, timezone, method)
                print(f"\nNew Day: {current_date}. Recalculating prayer times For Acton, CA:")
                for prayer, time_value in prayer_times.items():
                    if prayer != "date":
                        print(f"{prayer.capitalize()}: {time_value}")
            ...
            for prayer, time_value in prayer_times.items():
                if prayer == "date":  # Skip the 'date' entry
                    continue
                if now.time().hour == time_value.hour and now.time().minute == time_value.minute:
					# Print the current and prayer
                    current_time = now.strftime("%H:%M")
                    print(f"It's {current_time} time for {prayer}! Playing Adhan.")
...
if __name__ == "__main__":
    # Define Acton, California coordinates
    latitude, longitude, timezone = 34.48, -118.19, -8  # Pacific Time
    ...
    prayer_times_df = pd.DataFrame.from_dict(prayer_times, orient='index', columns=['Time'])
    # print("Today's Salah times for Acton, CA:")
    # print(prayer_times_df)

    # Path to MP3 file
    mp3_file = "Abdul-Basit.mp3"  # Replace with the path to your MP3 file

    # Monitor prayer times and play the MP3 file
    monitor_and_update_prayer_times(latitude, longitude, timezone, method, mp3_file)